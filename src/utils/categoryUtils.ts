...
// 키워드 기반 자동 카테고리 매핑 (2순위 - 카테고리 미선택 시에만 사용)
const CATEGORY_KEYWORDS: { [key: string]: string[] } = {
  ...
  '교통': ['택시', '버스', '지하철', '기차', 'KTX', '주유', '기름', '톨게이트', '고속도로', '주차', '카카오택시', '타다', '대리', '교통비', '티머니', '캐시비', '차량', '자동차'],
  '쇼핑': ['옷', '신발', '가방', '쇼핑', '백화점', '아울렛', '무신사', '지그재그', '쿠팡', '11번가', '지마켓', '네이버쇼핑', '의류', '악세사리', '화장품', '올리브영', '노트북', '컴퓨터', '전자제품'],
  '여가': ['영화', '게임', '넷플릭스', '유튜브', '웨이브', '티빙', '왓챠', '노래방', '볼링', '당구', 'PC방', '헬스', '운동', '골프', '등산', '여행', '숙소', '호텔', '펜션', '항공', '자전거', '스키', '스포츠'],
  '의료': ['병원', '약국', '약', '진료', '치과', '안과', '피부과', '정형외과', '내과', '의원', '클리닉', '건강', '영양제', '비타민'],
  '통신': ['통신', '휴대폰', '인터넷', 'SKT', 'KT', 'LG', '요금', '데이터'],
};

// 카테고리 추론 함수
// 1순위: 고객이 선택한 카테고리 그대로 사용
// 2순위: 카테고리 미선택 시에만 자동 매핑
export function inferCategory(description: string, existingCategory?: string): string {
  // 1순위: 고객이 카테고리를 선택했으면 그대로 사용
  if (existingCategory && CATEGORIES[existingCategory]) {
    return existingCategory;
  }

  // 2순위: 카테고리 미선택 시 키워드 기반 자동 매핑
