...
// [Phase 3] 보험설계 / DESIRE분석 / Executive Summary / Action Plan / 면책조항
...
const loadData = () => {
  ...
  const interests: string[] = b?.interests || [];
  ...
  const emFund = b?.emergencyFund||b?.assets?.emergency||0;
  const mReq = exp.living+exp.insurance+exp.loan;
  const emMon = mReq>0?Math.round(emFund/mReq*10)/10:0;
  const totAst = ast.total||d?.invest?.totalAssets||0;
  const dRatio = totAst>0?Math.round((dbt.totalDebt/totAst)*100):0;
  const annInc = inc.total*12; const annLoan = exp.loan*12;
  const dsr = annInc>0?Math.round((annLoan/annInc)*100):0;
  const savRate = inc.total>0?Math.round(((exp.saving+exp.pension)/inc.total)*100):0;
  const netAst = totAst-dbt.totalDebt;
  ...
  const insItems = [
    {key:'death',label:'사망',emoji:'💀',needed:insAI*3+insTD,prepared:prep.death||0,isSpecial:false,specialVal:''},
    {key:'disability',label:'장해',emoji:'🦽',needed:insAI*3+insTD,prepared:prep.disability||0,isSpecial:false,specialVal:''},
    {key:'cancer',label:'암진단',emoji:'🏥',needed:insAI*2,prepared:prep.cancer||0,isSpecial:false,specialVal:''},
    {key:'brain',label:'뇌질환',emoji:'🧠',needed:insAI,prepared:prep.brain||0,isSpecial:false,specialVal:''},
    {key:'heart',label:'심질환',emoji:'❤️',needed:insAI,prepared:prep.heart||0,isSpecial:false,specialVal:''},
    {key:'medical',label:'실비',emoji:'💊',needed:5000,prepared:prep.medical||0,isSpecial:false,specialVal:''},
    {key:'hospital',label:'입원수술',emoji:'🏨',needed:1,prepared:isIns(prep.hospital||'X')?1:0,isSpecial:true,specialVal:String(prep.hospital||'X')},
    {key:'dementia',label:'치매간병',emoji:'🧓',needed:1,prepared:isIns(prep.dementia||'X')?1:0,isSpecial:true,specialVal:String(prep.dementia||'X')},
  ];
  const insLack=insItems.filter(i=>i.isSpecial?i.prepared===0:i.prepared<i.needed).length;
  const insNeedT=insItems.filter(i=>!i.isSpecial).reduce((s,i)=>s+i.needed,0);
  ...
  const dStages = [
    ...
    {letter:'S',name:'Savings',kr:'저축',emoji:'💰'},
    {letter:'I',name:'Investment',kr:'투자',emoji:'📈'},
    {letter:'R',name:'Retirement',kr:'은퇴준비',emoji:'🏠'},
    {letter:'E',name:'Enjoy',kr:'경제적자유',emoji:'🎉'},
  ];
  ...
  return {
    pi, interests, goal, inc, exp, ast, dbt, mortT, credT, othDT, emFund, mReq, emMon, totAst, dRatio, dsr, savRate, netAst, reAst,
    save:{purpose:svPurp,targetYears:svYrs,targetAmount:svAmt,monthlySavingRequired:svMon},
    retire:{currentAge:rAge,retireAge:rRAge,yearsToRetire:yToR,retireYears:rYrs,monthlyLivingExpense:rExp,expectedNationalPension:rNP,currentPersonalPension:rPP,expectedRetirementLumpSum:rLump,requiredMonthly:rExp,preparedMonthly:rPrep,monthlySavingForRetire:rLumpM,totalPreparedMonthly:rTotPrep,retirementReadyRate:rRate,monthlyShortfall:rShort,totalRequiredRetireFund:rTotNeed,additionalMonthlySaving:rAddMon},
    invest:{currentAge:iAge,monthlyIncome:iInc,totalAssets:iTotA,totalDebt:iTotD,netAsset:iNet,annualIncome:iAnnInc,wealthIndex:wIdx,portfolio:pf,portfolioTotal:pfTot,isDualIncome:isDual,recommendedEmergency:recEm},
    tax:{annualSalary:txSal,determinedTax:txDet,prepaidTax:txPre,taxRefund:txRef,effectiveTaxRate:txEff,inherit:{totalAssets:ihA,totalDebts:ihD,hasSpouse:ihSp,childrenCount:ihCh,basicDeduction:ihBD,spouseDeduction:ihSD,childDeduction:ihCD,totalDeduction:ihTD,taxableAmount:ihTax,tax:ihRes.tax,rate:ihRes.rate,bracket:ihRes.bracket}},
    insurance:{items:insItems,lackCount:insLack,totalNeeded:insNeedT,totalPrepared:insPrepT,overallRate:insRate,annualIncome:insAI},
    desire:{currentStage:dCur,stageName:dName,stageEmoji:dEmoji,stageDesc:dDesc,stages:dStages},
  };
};
...
const FinancialReport = ({ userName, onClose }: Props) => {
  ...
  const retG = getRetireGrade(data.retire.retirementReadyRate); const wG = getWealthGrade(data.invest.wealthIndex);
  const insG = getInsuranceGrade(data.insurance.overallRate);

  const reT = data.reAst.residential+data.reAst.investment;
  const reR = data.totAst>0?Math.round((reT/data.totAst)*100):0;
          ...
          <div className={`${printMode==='a4'?'p-[15mm]':'p-4 pb-20'} space-y-5`}>
            ...
            <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 text-white min-h-[200px] flex flex-col justify-between print:break-after-page">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-1"><img src={LOGO_URL} alt="AI머니야" className="h-7 rounded"/><span className="text-[10px] text-teal-300 font-medium tracking-wider">AI MONEYA</span></div>
                <p className="text-[10px] text-slate-400 mt-2">{dateStr} 기준</p>
              </div>
              <div className="relative z-10 mt-6">
                <h1 className="text-2xl font-extrabold tracking-tight leading-snug">{nm}님의<br/><span className="text-teal-300">종합재무설계</span> 리포트</h1>
                <p className="text-xs text-slate-400 mt-2">금융집짓기® 기반 | 오원트금융연구소</p>
              </div>
              <div className="relative z-10 mt-4 flex items-center gap-2">
                <img src={PROFILE_IMAGE_URL} alt="오상열 CFP" className="w-8 h-8 rounded-full border border-slate-600 object-cover"/>
                <div><p className="text-[10px] font-semibold text-slate-300">오상열 CFP</p><p className="text-[9px] text-slate-500">오원트금융연구소 대표</p></div>
              </div>
            </section>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative w-20 h-20 flex-shrink-0">
                    <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                      ...
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke={overallGrade==='A'?'#059669':overallGrade==='B'?'#0891b2':overallGrade==='C'?'#d97706':'#dc2626'} strokeWidth="3" strokeDasharray={`${overallScore} ${100-overallScore}`} strokeLinecap="round"/>
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-xl font-extrabold" style={{color:overallGrade==='A'?'#059669':overallGrade==='B'?'#0891b2':overallGrade==='C'?'#d97706':'#dc2626'}}>{overallGrade}</span>
                      <span className="text-[9px] text-slate-400">{overallScore}점</span>
                    </div>
                  </div>
                  <div className="flex-1 space-y-1">
                    <p className="text-sm font-bold text-slate-700">{nm}님의 종합 재무건강도</p>
                    <p className="text-[10px] text-slate-400">7개 영역 분석 기반 종합 평가</p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {[{l:'부채',g:debtG},{l:'저축',g:savG},{l:'은퇴',g:retG},{l:'투자',g:wG},{l:'보험',g:insG}].map(x=>(
                        <span key={x.l} className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{backgroundColor:x.g.color+'15',color:x.g.color}}>{x.l} {x.g.grade}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl p-3 border border-slate-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-700">DESIRE {data.desire.currentStage}단계: {data.desire.stageName}</p>
                      <p className="text-[10px] text-slate-400">{data.desire.stageDesc}</p>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    {data.desire.stages.map((s,i)=>(
                      <div key={i} className={`flex-1 h-2 rounded-full ${i<data.desire.currentStage?'bg-teal-400':i===data.desire.currentStage-1?'bg-amber-400':'bg-slate-200'}`} title={`${s.letter}: ${s.kr}`}/>
                    ))}
                  </div>
                </div>
                {/* 핵심 KPI 4개 */}
                <div className="grid grid-cols-4 gap-2">
                  {[{l:'순자산',v:fmt.eok(data.netAst),c:'#6366f1'},{l:'부자지수',v:String(data.invest.wealthIndex),c:'#8b5cf6'},{l:'은퇴준비',v:`${data.retire.retirementReadyRate}%`,c:'#0891b2'},{l:'보험보장',v:`${data.insurance.overallRate}%`,c:'#059669'}].map(x=>(
                    <div key={x.l} className="text-center p-2 rounded-lg" style={{backgroundColor:x.c+'10'}}>
                      ...
                      <p className="text-sm font-extrabold" style={{color:x.c}}>{x.v}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <Sec num="01" title="인적사항" color="indigo">
              <div className="grid grid-cols-3 gap-2">
                ...
                <IC l="수입형태" v={data.pi.dualIncome} c="emerald"/><IC l="직업" v={data.pi.job} c="slate"/><IC l="가족수" v={`${data.pi.familyCount}명`} c="amber"/>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <GradientCard label="월수입" value={fmt.manwon(data.inc.total)} from="emerald" to="teal"/>
                <GradientCard label="월지출" value={fmt.manwon(data.exp.total)} from="rose" to="orange"/>
                <GradientCard label="총자산" value={fmt.eok(data.totAst)} from="blue" to="indigo"/>
                <GradientCard label="순자산" value={fmt.eok(data.netAst)} from="violet" to="purple" sub={`부채 ${fmt.eok(data.dbt.totalDebt)}`}/>
              </div>
            ...
            <Sec num="02" title="경제적 관심사 & 재무목표" color="teal">
              {data.interests.length>0?(
                <div className="space-y-2">
                  <p className="text-[10px] text-slate-400 font-semibold">🎯 관심사 (우선순위)</p>
                  <div className="flex flex-wrap gap-2">{data.interests.map((k,i)=>{const it=interestLabels[k]||{emoji:'🔹',label:k};return(<div key={k} className="flex items-center gap-1.5 bg-slate-50 rounded-full px-3 py-1.5 border border-slate-100"><span className="text-[10px] text-teal-600 font-bold">{i+1}</span><span className="text-xs">{it.emoji}</span><span className="text-xs font-medium text-slate-700">{it.label}</span></div>);})}</div>
                </div>
              ):(<p className="text-xs text-slate-400 text-center py-4">관심사 미등록</p>)}
            ...
            <Sec num="06" title="투자설계" color="violet" pill={wG}>
              ...
              {data.invest.portfolioTotal>0&&(<BarChart title="📊 금융자산 포트폴리오" items={pfItems} max={pfMax} showPct pctFn={pfPct}/>)}
            ...
            <Sec num="09" title="보험설계" color="pink" pill={insG}>
              ...
              <div className="space-y-2">
                ...
                <div className="border border-slate-100 rounded-xl overflow-hidden">
                  <div className="bg-slate-50 px-3 py-2 grid grid-cols-4 text-[9px] text-slate-400 font-semibold"><span>보장</span><span className="text-right">필요자금</span><span className="text-right">준비자금</span><span className="text-right">상태</span></div>
                  {data.insurance.items.map(item => {
                    const isOk = item.isSpecial ? item.prepared>0 : item.prepared>=item.needed;
                    const ratio = item.isSpecial ? (item.prepared>0?100:0) : (item.needed>0?Math.round((item.prepared/item.needed)*100):0);
                    return (
                      <div key={item.key} className={`px-3 py-2 grid grid-cols-4 items-center border-t border-slate-50 ${isOk?'':'bg-rose-50/50'}`}>
                        <span className="text-[10px] text-slate-600 font-medium">{item.emoji} {item.label}</span>
                        <span className="text-[10px] text-slate-500 text-right">{item.isSpecial?'특약':fmt.manwon(item.needed)}</span>
                        <span className={`text-[10px] text-right font-semibold ${isOk?'text-emerald-600':'text-rose-600'}`}>{item.isSpecial?(item.prepared>0?'가입':'미가입'):fmt.manwon(item.prepared)}</span>
                        <div className="flex justify-end">
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isOk?'bg-emerald-100 text-emerald-700':'bg-rose-100 text-rose-700'}`}>{isOk?'✓':ratio+'%'}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-pink-50 rounded-xl p-3 border border-pink-100">
                ...
                <div className="h-3 bg-pink-100 rounded-full overflow-hidden relative">
                  <div className="h-full rounded-full bg-gradient-to-r from-pink-400 to-rose-500" style={{width:`${Math.min(100,data.insurance.overallRate)}%`}}/>
                  <div className="absolute top-0 h-full w-0.5 bg-red-600" style={{left:'100%'}}/>
                </div>
                <p className="text-[9px] text-slate-400 mt-1">빨간선 = 필요자금 100% 충족</p>
              </div>
            ...
            <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 rounded-2xl shadow-sm overflow-hidden text-white print:break-before-page">
              <div className="px-5 py-3.5 border-b border-white/10 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold">DESIRE 로드맵</h2>
              </div>
              <div className="p-4 space-y-4">
                <div className="text-center">
                  <span className="text-4xl">{data.desire.stageEmoji}</span>
                ...
                <div className="flex gap-1.5">
                  {data.desire.stages.map((s,i)=>{
                    ...
                    const current = i === data.desire.currentStage - 1;
                    return (
                      <div key={i} className="flex-1 text-center">
                        <div className={`w-10 h-10 rounded-xl mx-auto flex items-center justify-center text-sm font-extrabold border-2 ${done?'bg-emerald-500/30 border-emerald-400 text-emerald-300':current?'bg-amber-500/30 border-amber-400 text-amber-300':'bg-white/5 border-white/10 text-slate-500'}`}>{s.letter}</div>
                        <p className="text-[8px] text-slate-400 mt-1">{s.kr}</p>
                      </div>
                    );
                  })}
                </div>
                <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                  <p className="text-[10px] text-teal-300 font-semibold mb-1">📋 DESIRE 단계별 의미</p>
                  <div className="space-y-1">{data.desire.stages.map((s,i)=>(<div key={i} className="flex items-center gap-2 text-[10px]"><span className={`font-bold ${i<data.desire.currentStage?'text-emerald-400':i===data.desire.currentStage-1?'text-amber-400':'text-slate-500'}`}>{s.letter}</span><span className="text-slate-400">{s.name}</span><span className="text-slate-500">— {s.kr}</span></div>))}</div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden print:break-before-page">
              <div className="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-800">🎯 맞춤 Action Plan</h2>
              </div>
              <div className="p-4 space-y-3">
                {actionPlan.map(ap => (
                  <div key={ap.priority} className="flex items-start gap-3 bg-slate-50 rounded-xl p-3 border border-slate-100">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-teal-400 to-emerald-500 flex items-center justify-center text-white text-sm font-extrabold flex-shrink-0">{ap.priority}</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5"><span>{ap.emoji}</span><span className="text-xs font-bold text-slate-700">{ap.action}</span><span className="text-[9px] text-slate-400">({ap.area})</span></div>
                      <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">{ap.detail}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-slate-100 rounded-xl p-4 text-center space-y-1">
              ...
              <p className="text-[9px] text-slate-300">© {today.getFullYear()} 오원트금융연구소 | AI머니야</p>
            </div>
...
const GradientCard = ({label,value,from,to,sub}:{label:string;value:string;from:string;to:string;sub?:string}) => (
  <div className={`bg-gradient-to-br from-${from}-50 to-${to}-50 rounded-xl p-3 border border-${from}-100`}>
    <p className={`text-[10px] text-${from}-500 font-medium`}>{label}</p>
    <p className={`text-lg font-extrabold text-${from}-700`}>{value}</p>
    {sub&&<p className={`text-[9px] text-${from}-400`}>{sub}</p>}
  </div>
);

const GB = ({g}:{g:{grade:string;label:string;color:string;bg:string}}) => (
  <div className="flex flex-col items-center gap-1 flex-shrink-0"><div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-extrabold border-2" style={{backgroundColor:g.bg,color:g.color,borderColor:g.color+'40'}}>{g.grade}</div><span className="text-[10px] font-semibold" style={{color:g.color}}>{g.label}</span></div>
);

const DR = ({c,l,n,a,r}:{c:string;l:string;n:number;a:number;r:number}) => (
  <div className="flex items-center justify-between"><div className="flex items-center gap-2"><div className={`w-2.5 h-2.5 rounded-full ${c}`}/><span className="text-xs text-slate-600">{l} ({n}건)</span></div><div className="flex items-center gap-3"><span className="text-xs font-semibold text-slate-700">{fmt.eok(a)}</span><span className="text-[10px] text-slate-400 w-8 text-right">{r}%</span></div></div>
);

const EmergencyBox = ({fund,months,required,grade,isDual,recAmt}:{fund:number;months:number;required:number;grade:{grade:string;label:string;color:string};isDual:boolean;recAmt:number}) => (
  <div className="bg-slate-50 rounded-xl p-3 space-y-2">
    <div className="flex items-center justify-between"><p className="text-[10px] text-slate-400 font-semibold">🛡️ 비상예비자금</p><span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{backgroundColor:grade.color+'15',color:grade.color}}>{grade.grade} {grade.label}</span></div>
    <div className="flex items-end justify-between"><div><p className="text-lg font-extrabold" style={{color:grade.color}}>{fmt.manwon(fund)}</p><p className="text-[10px] text-slate-400">월필수지출 {fmt.manwon(required)} × {months}개월</p></div><p className="text-[10px] text-slate-500">권장: {isDual?'3':'6'}개월 ({fmt.manwon(recAmt)})</p></div>
  </div>
);

const TimelineBar = ({age,retireAge}:{age:number;retireAge:number}) => (
  <div className="bg-slate-50 rounded-xl p-3"><p className="text-[10px] text-slate-400 font-semibold mb-2">📅 생애주기</p><div className="relative h-2 bg-slate-200 rounded-full overflow-hidden"><div className="absolute left-0 top-0 h-full bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full" style={{width:`${Math.min(100,age>0?(age/90)*100:0)}%`}}/>{retireAge>0&&<div className="absolute top-0 h-full w-0.5 bg-amber-500" style={{left:`${(retireAge/90)*100}%`}}/>}</div><div className="flex justify-between mt-1.5"><span className="text-[9px] text-teal-600 font-semibold">현재 {age}세</span><span className="text-[9px] text-amber-600 font-semibold">은퇴 {retireAge}세</span><span className="text-[9px] text-slate-400">90세</span></div></div>
);

const BarChart = ({title,items,max,showPct,pctFn}:{title:string;items:{l:string;v:number;c:string}[];max:number;showPct?:boolean;pctFn?:(v:number)=>number}) => (
  <div><p className="text-[10px] text-slate-400 font-semibold mb-2">{title}</p><div className="space-y-2">{items.map(i=>(<div key={i.l} className="flex items-center gap-2"><span className="text-[10px] text-slate-500 w-12 text-right flex-shrink-0">{i.l}</span><div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{width:`${(i.v/max)*100}%`,backgroundColor:i.c}}/></div><div className="flex items-center gap-1 w-20 justify-end flex-shrink-0"><span className="text-[10px] font-semibold text-slate-600">{fmt.manwon(i.v)}</span>{showPct&&pctFn&&<span className="text-[9px] text-slate-400">({pctFn(i.v)}%)</span>}</div></div>))}</div></div>
);
...
const DonutSection = ({label,ratio,total,other,warn}:{label:string;ratio:number;total:number;other:number;warn?:string}) => (
...
const InheritSection = ({inh}:{inh:any}) => (
